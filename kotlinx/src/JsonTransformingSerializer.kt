...
public abstract class JsonTransformingSerializer<T : Any?>(
    ...
    final override fun deserialize(decoder: Decoder): T {
        val input = decoder as ZeroJsonDecoder
        val element = input.decodeJsonElement()
        return input.zeroJson.decodeFromJsonElement(tSerializer, transformDeserialize(element))
    }
...
private fun Any.throwUnexpectedType(reciever: String, expected: String): Nothing =
    throw IllegalStateException(
        "This serializer can be used only with Json format." +
        "Expected $reciever to be $expected, got ${this::class}"
    )

private fun Any.throwExpectedJsonDecoder(): Nothing = throwUnexpectedType("Decoder", "JsonDecoder")
private fun Any.throwExpectedJsonEncoder(): Nothing = throwUnexpectedType("Encoder", "JsonEncoder")
private fun Any.throwExpectedZeroJsonDecoder(): Nothing = throwUnexpectedType("Decoder", "ZeroJsonDecoder")
private fun Any.throwExpectedZeroJsonEncoder(): Nothing = throwUnexpectedType("Encoder", "ZeroJsonEncoder")

internal fun Decoder.asJsonDecoder(): JsonDecoder =
    this as? JsonDecoder ?: throwExpectedJsonDecoder()

internal fun Encoder.asJsonEncoder(): JsonEncoder =
    this as? JsonEncoder ?: throwExpectedJsonEncoder()