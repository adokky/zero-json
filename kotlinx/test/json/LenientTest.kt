...
class LenientTest : JsonTestBase() {
    ...
    fun testUnquotedStringValue() = parametrizedTest {
        val json = """{"i":1, "l":2, "b":true, "s":string}"""
        assertFailsWithSerial("ZeroJsonDecodingException") { default.decodeFromString(Holder.serializer(), json, it) }
        assertEquals(value, lenient.decodeFromString(Holder.serializer(), json, it))
    }
    ...
    fun testUnquotedStringInArray() = parametrizedTest {
        val json = """{"l":[1, 2, ss]}"""
        assertFailsWithSerial("ZeroJsonDecodingException") { default.decodeFromString(ListHolder.serializer(), json, it) }
    ...
    fun testNullsProhibited() = parametrizedTest {
        ...
        assertFailsWithSerial("ZeroJsonDecodingException") { lenient.decodeFromString<StringWrapper>("""{"s":null}""", it) }
    }
