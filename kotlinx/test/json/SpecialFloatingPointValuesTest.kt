...
import kotlinx.serialization.test.noJs
...
class SpecialFloatingPointValuesTest : JsonTestBase() {
    ...
    private fun test(box: Box, expected: String, jsonTestingMode: JsonTestingMode) {
        val expectedMessage = if (jsonTestingMode == JsonTestingMode.TREE) "expected" else "Unexpected special floating-point value"
        assertFailsWithSerialMessage("SerializationException", expectedMessage) {
            default.encodeToString(Box.serializer(), box, jsonTestingMode)
        }
        ...
        assertFailsWithSerialMessage("ZeroJsonDecodingException", expectedMessage) {
            default.decodeFromString(Box.serializer(), expected, jsonTestingMode)
        }
    }
}
