...
class JsonNamingStrategyTest : JsonTestBase() {
    ...
    fun testNamingStrategyPrioritizesOverAlternative() {
        ...
        parametrizedTest { mode ->
            assertFailsWithMessage<SerializationException>("Element with name 'test_case' appeared twice in class with serial name 'kotlinx.serialization.features.JsonNamingStrategyTest.CollisionCheckPrimary'") {
                json.decodeFromString<CollisionCheckPrimary>("""{"test_case":"a"}""", mode)
            }
        }
        parametrizedTest { mode ->
            assertFailsWithMessage<SerializationException>("Element with name 'test_case' appeared twice in class with serial name 'kotlinx.serialization.features.JsonNamingStrategyTest.CollisionCheckAlternate'") {
