...
class JsonEnumsCaseInsensitiveTest: JsonTestBase() {
    ...
    fun testDocSample() {
        ...
        j.decodeFromString<Outer>("""{"enums":["value_A", "alternative"]}""").enums
    }
    ...
    fun testFeatureDisabledThrowsWithoutCoercing() = parametrizedTest { mode ->
        ...
        assertFailsWithMessage<SerializationException>("unknown entry 'BAz'") {
    ...
    fun testLowercaseClashThrowsException() = parametrizedTest { mode ->
        val expectedMessage = "Element with name 'BAD' appeared twice in class with serial name 'kotlinx.serialization.features.JsonEnumsCaseInsensitiveTest.BadEnum'"
