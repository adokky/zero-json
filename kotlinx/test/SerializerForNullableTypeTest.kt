...
class SerializerForNullableTypeTest : JsonTestBase() {
    ...
    object NullableLongSerializer : KSerializer<Long?> {
        @SerialName("NLS")
        @Serializable
        data class NLS(val initialized: Boolean, val value: Long? = 0)
        ...
        override fun serialize(encoder: Encoder, value: Long?) {
            val opt = NLS(value != null, value)
            encoder.encodeSerializableValue(NLS.serializer(), opt)
        }

        override fun deserialize(decoder: Decoder): Long? {
            val value = decoder.decodeSerializableValue(NLS.serializer())
