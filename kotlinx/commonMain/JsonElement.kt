 ...
 */
@Serializable(with = dev.dokky.zerojson.JsonElementSerializer::class)
 ...
 */
@Serializable(with = dev.dokky.zerojson.JsonPrimitiveSerializer::class)
 ...
 * @throws SerializationException if `value == "null"`
 */
@ExperimentalSerializationApi
...
public fun JsonUnquotedLiteral(value: String?): JsonPrimitive {
    return when (value) {
        null -> JsonNull
        JsonNull.content -> throw SerializationException("Creating a literal unquoted value of 'null' is forbidden. If you want to create JSON null literal, use JsonNull object, otherwise, use JsonPrimitive")
...
internal class JsonLiteral internal constructor(
    ...
    override fun toString(): String = if (isString) content.toJsonString() else content
 ...
 */
@Serializable(with = dev.dokky.zerojson.JsonNullSerializer::class)
 ...
 */
@Serializable(with = dev.dokky.zerojson.JsonObjectSerializer::class)
...
@Serializable(with = dev.dokky.zerojson.JsonArraySerializer::class)
 ...
 * @throws NumberFormatException if current element is not a valid representation of number
 */
public val JsonPrimitive.long: Long get() = parseLongImpl()
...
private inline fun <T> exceptionToNull(f: () -> T): T? =
    try { f() } catch (e: IllegalArgumentException) { null }
...
internal fun JsonPrimitive.parseLongImpl(): Long {
    val reader = JsonReader.startReadingFrom(content)
    val result = try {
        reader.readLong(skipWhitespace = false)
    } catch (e: ZeroJsonDecodingException) {
        throw NumberFormatException(e.message)
    }
    if (reader.nextCodePoint >= 0) reader.fail("malformed number")
    return result
}