...
class SerialNameCollisionInSealedClassesTest {
    ...
    private fun Json(discriminator: String) = Json { classDiscriminator = discriminator }

    @Test
    fun testCollisionWithDiscriminator() {
        assertFailsWithSerialMessage("SerializationException", "discriminator") {
            Json("type").encodeToString(Base.serializer(), Base.Child("a"))
        }
        assertFailsWithSerialMessage("SerializationException", "discriminator") {
            Json("type2").encodeToString(Base.serializer(), Base.Child("a"))
        }
