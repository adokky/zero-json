...
class JsonElementDecodingTest : JsonTestBase() {
    ...
    fun testDeepRecursion() {
        ...
        json = json.replace("%", "0")
        assertFailsWith<JsonMaxDepthReachedException> { Json.parseToJsonElement(json) }
    }

    private open class NullAsElementSerializer<T : Any>(private val serializer: KSerializer<T>, val nullElement: T) : KSerializer<T?> {
        final override val descriptor: SerialDescriptor get() = serializer.descriptor.nullable
    ...
    fun testLiteralIncorrectParsing() {
        ...
        assertFailsWithMessage<ZeroJsonDecodingException>("malformed number") {
