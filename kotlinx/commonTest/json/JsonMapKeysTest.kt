...
class JsonMapKeysTest : JsonTestBase() {
    ...
    @Test
    fun testStructuredMapKeysShouldBeProhibitedByDefault() = parametrizedTest { mode ->
        verifyProhibition(WithComplexKey(mapOf(IntData(42) to "42")), mode)
        verifyProhibition(WithComplexValueKey(mapOf(ComplexCarrier(IntData(42)) to "42")), mode)
    }

    private inline fun <reified T: Any> verifyProhibition(value: T, mode: JsonTestingMode) {
        assertFailsWithSerialMessage("SerializationException", "can't be used in JSON as a key in the map") {
            Json.encodeToString(value, mode)
        }
    }
