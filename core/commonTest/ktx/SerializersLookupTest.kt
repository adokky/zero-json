...
class SerializersLookupTest : JsonTestBase() {
    ...
    fun testTriple() {
        ...
        assertSerializedWithType("""{"first":"1","second":2,"third":{"value":42}}""", myTriple)
    }
    ...
    fun testCustomGeneric() {
        ...
        assertSerializedWithType("""{"value":42}""", intBox)
        val dataBox = Box(StringData("foo"))
        assertSerializedWithType("""{"value":{"data":"foo"}}""", dataBox)
    }

    @Test
    fun testRecursiveGeneric() {
        val boxBox = Box(Box(Box(IntData(42))))
        assertSerializedWithType("""{"value":{"value":{"value":{"intV":42}}}}""", boxBox)
    }

    @Test
    fun testMixedGeneric() {
        val listOfBoxes = listOf(Box("foo"), Box("bar"))
        assertSerializedWithType("""[{"value":"foo"},{"value":"bar"}]""", listOfBoxes)
        val boxedList = Box(listOf("foo", "bar"))
        assertSerializedWithType("""{"value":["foo","bar"]}""", boxedList)
    }
    ...
    fun testReferenceArrays() {
        ...
        assertSerializedWithType("""[{"value":"foo"}]""", Array(1) { Box("foo") }, default)
        assertSerializedWithType("""[[{"value":"foo"}]]""", Array(1) { Array(1) { Box("foo") } }, default)
    }
    ...
    @Suppress("unused")
    private class IntBox(val i: Int)

    private class CustomIntSerializer(isNullable: Boolean) : KSerializer<IntBox?> {
    ...
    @Suppress("unused")
    private class GenericHolder<T>(value: T)

    private class GenericSerializer<T>(typeSerializer: KSerializer<T>) : KSerializer<GenericHolder<T>> {
        override val descriptor: SerialDescriptor =
    ...
    @Suppress("unused")
    private class NonSerializableBox<T>(val boxed: T)
