...
class JsonTreeTest : JsonTestBase() {
    ...
    @Serializable
    private data class DataWrapper(val s: String, val d: Data?)

    @Serializable
    private data class DataWrapperOptional(val s: String, val d: Data? = null)

    @Serializable
    private data class IntList(val l: List<Int>)

    @Serializable
    private data class DataList(val l: List<Data>)

    @Serializable
    private data class ListOfLists(val l: List<List<Data>>)

    @Serializable
    private data class MapWrapper(val m: Map<String, Int>)

    @Serializable
    private data class ComplexMapWrapper(val m: Map<String, Data>)

    @Serializable
    private data class AllTypes(
        val b: Byte,
        val s: Short,
        val i: Int,
        val f: Float,
    ...
    private fun prepare(input: String): JsonElement {
        ...
        val actual = lenient.parseToJsonElement(input)
    ...
    fun testReadTreeNullable() {
        ...
        assertFailsWith<MissingFieldException> { lenient.decodeFromJsonElement(DataWrapper.serializer(), tree2) }
    }
    ...
    private inline fun <reified T: Any> writeAndTest(obj: T, serial: KSerializer<T>): Pair<JsonElement, T> {
        val tree = ktxJson.encodeToJsonElement(serial, obj)
