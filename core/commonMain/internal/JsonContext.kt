...
internal class JsonContext(
    ...
    private fun <T> decode(
        ...
        try {
            ...
            if (e.path == null) e.path = jsonPathFromDecoderStack()
            throw e
        } finally {
            input.resetInput()
            decoder.close()
    ...
    fun <T> decodeFromJsonElement(deserializer: DeserializationStrategy<T>, element: JsonElement): T {
        ...
        try {
            ...
            if (e.path == null) e.path = jsonTreePath()
            throw e
...
//            )
    ...
    private fun jsonPathFromDecoderStack(): String = buildString(stringBuilder) {
        textDecodingStack.currentJsonPath(reader, this)
    }

    private fun jsonTreePath(): String = buildString(stringBuilder) { jsonTreePath(this) }

    private fun jsonTreePath(output: StringBuilder) = output.run {
        append('$')
        for (i in 1 until treeDecoderStack.acquired) {
            ...
            when(decoder.descriptor.kind) {
                StructureKind.MAP -> decoder.currentKey?.let { append('.').append(it) } ?: break
                ...
                StructureKind.CLASS -> {
                    ...
                    append('.').append(decoder.currentKey)
                }
                else -> break
            }
        }
